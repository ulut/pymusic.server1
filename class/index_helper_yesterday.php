<?php
	...
	$yesterday_dateStart[] = strtotime('yesterday');
	$yesterday_dateStart[] = strtotime('-2 days');
	
	$yesterday_dateLast[] = strtotime('today');
	$yesterday_dateLast[] = strtotime('yesterday');
	
	$yesterday_leader_singer[] = apc_fetch('yesterday_leader_singer_'.$yesterday_dateStart[0]);
	$yesterday_leader_singer[] = apc_fetch('yesterday_leader_singer_'.$yesterday_dateStart[1]);
	
	$yesterday_leader_song[] = apc_fetch('yesterday_leader_song_'.$yesterday_dateStart[0]);
	$yesterday_leader_song[] = apc_fetch('yesterday_leader_song_'.$yesterday_dateStart[1]);

	//////////////
	/// SINGER STATS
	//////////////
	
	$leader_singer_count = count($yesterday_leader_singer)-1;
	
	for($i=$leader_singer_count; $i>-1; $i--){
		$limit = 100;

		if (!$yesterday_leader_singer[$i]) {
			...
			and pm.date_played >= '".date("Y-m-d",$yesterday_dateStart[$i])."' and pm.date_played < '".date("Y-m-d",$yesterday_dateLast[$i])."' 
			...
			$yesterday_leader_singer[$i] = $leader_singer_result;
			apc_store('yesterday_leader_singer_'.$yesterday_dateStart[$i],new ArrayObject($leader_singer_result));
			//if ($i==0) apc_store('day_leader_singer_1_'.$dateStart[$i],new ArrayObject($leader_singer_result),60);
			//else apc_store('day_leader_singer_'.$dateStart[$i],new ArrayObject($leader_singer_result));
		}
	}
	...
	$leader_song_count = count($yesterday_leader_song)-1;
	
	for($i=$leader_song_count; $i>-1; $i--){
		if (!$yesterday_leader_song[$i]) {
			$limit = 100;
			...
			$yesterday_leader_song[$i] = $leader_song_result;
			apc_store('yesterday_leader_song_'.$yesterday_dateStart[$i],new ArrayObject($leader_song_result));
			//if ($i==0) apc_store('day_leader_song_1_'.$dateStart[$i],new ArrayObject($leader_song_result),60);
			//else apc_store('day_leader_song_'.$dateStart[$i],new ArrayObject($leader_song_result));
		}
	}
?>