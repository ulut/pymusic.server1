...
<div id="page-wrapper">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">Жаңы ырлардын тизмеси</h1>
            </div>
        </div>
...
foreach($res as $key=>$r){
    ...
    $counter = 0;
    while($res[$temp]['id'] == $res[$temp+1]['id']){
        $counter++;
        $temp++;
    }
    ...
    <div class="row">
        ...
        <div class="col-md-3">
            ...
            <a onclick="return confirm('Вы уверены?')" href="unproved_songs.php?id=<?=$r['id'];?>" class="btn btn-info col-md-4" >Удалить</a>
            <?php
            if($r['real_name']){

            ?>
            <a href="unproved_songs.php?approve_id=<?=$r['id'];?>" class="btn btn-info col-md-4">Подтвердить</a>
            <?php
            }
            ?>
        </div>
